...
def get_constant_scheduler(optim: Optimizer, last_epoch=-1):
    """
    get a constant scheduler of the lr
    """
    return LambdaLR(optim, lr_lambda=lambda _: 1., last_epoch=last_epoch)

def get_constant_scheduler_with_warmup(optim: Optimizer, num_warmup_step: float, last_epoch: int =-1):
    """
...
def get_linear_scheduler_with_warmup(optim: Optimizer,
    ...
    def lr_lambda(current_step):
        if current_step < num_warmup_step:
            return float(current_step) / float(max(1.0, num_warmup_step))
        return max(
            0.0, float(num_training_step - current_step) / float(max(1., num_training_step - num_warmup_step))
        )
    return LambdaLR(optim, lr_lambda, last_epoch)


def get_cosine_scheduler_with_warmup(optim: Optimizer,
                                     num_warmup_step: float,
                                     num_training_step: int,
    ...
    def lr_lambda(current_step):
        ...
        return max(0., .5 * (1. + math.cos(math.pi * float(num_cycles) * 2.0 * progress)))
    return LambdaLR(optim, lr_lambda, last_epoch)
